	...
	public void copy(int id) throws IOException, SQLException, ClassNotFoundException {
		...
		while (rs_getFromLog.next()) {
			...
			PreparedStatement ps_getDataFromConfig = connection
					.prepareStatement("SELECT table_name,table_warehouse,filed_name FROM table_config where id = "
							+ rs_getFromLog.getInt(1));
			...
			while (rs_getDataFromConfig.next()) {
				...
				while (rs_checkTableInStaging.next()) {
					// Kiem tra ten table trong staging lay trong config co ton tai trong database
					// staging
				...
				if (existInDW && existInStaging) {
					...
					if (countColOfDW == countColOfStaging) {
						...
						if (count == countColOfDW) {
							...
							String sql = "INSERT INTO " + rs_getDataFromConfig.getString(2) + "("
									+ rs_getDataFromConfig.getString(3).replaceAll("stt,", "") + ")" + " VALUES(";
							...
							while (rs_getAllFromStaging.next()) {
								...
								for (int i = 1; i < count; i++) {
									...
									if (rsmd_getAllFromDW.getColumnTypeName(i + 1).contains("INT")) {
										// Neu du lieu trong
										if (rs_getAllFromStaging.getString(i + 1) == null
												|| rs_getAllFromStaging.getString(i + 1).equals("")
												|| rs_getAllFromStaging.getString(i + 1).equals(" ")) {
											// Cho mac dinh la 0
											ps_copyToDW.setInt(i, 0);
										} else {
											ps_copyToDW.setInt(i, convertToInt(rs_getAllFromStaging.getString(i + 1)));
										}
									}
									// Neu cot co kieu du lieu la varchar
									if (rsmd_getAllFromDW.getColumnTypeName(i + 1).contains("VARCHAR")) {
										// Neu du lieu trong
										if (rs_getAllFromStaging.getString(i + 1) == null
												|| rs_getAllFromStaging.getString(i + 1).equals("")
												|| rs_getAllFromStaging.getString(i + 1).equals(" ")) {
											...
											ps_copyToDW.setString(i, rs_getAllFromStaging.getString(i + 1));
										}
									}
									// Neu cot co kieu du lieu la date
									if (rsmd_getAllFromDW.getColumnTypeName(i + 1).contains("DATE")) {
										// Neu du lieu trong
										if (rs_getAllFromStaging.getString(i + 1) == null
												|| rs_getAllFromStaging.getString(i + 1).equals("")
												|| rs_getAllFromStaging.getString(i + 1).equals(" ")) {
											...
											ps_copyToDW.setDate(i,
													convertToDate(convertToYMD(rs_getAllFromStaging.getString(i + 1))));
										}
									}
								}
							...
							// Update status le log
							PreparedStatement ps_setStatus = connection
									.prepareStatement("UPDATE table_log set status = 'Copy_OK' where id=" + id);
							ps_setStatus.executeUpdate();
